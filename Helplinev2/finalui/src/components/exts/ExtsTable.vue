<template>
  <div class="rounded-xl shadow-xl overflow-auto border" :class="isDarkMode
    ...
    <div v-if="!exts || exts.length === 0" class="p-4 text-center"
      :class="isDarkMode ? 'text-gray-500' : 'text-gray-500'">
      No extension history records found.
    </div>
  </div>
</template>
...
const props = defineProps({
  exts: { type: Array, required: true },
  extsStore: { type: Object, required: true }
})

// Map raw array rows to named fields using exts_k
const rows = computed(() => {
  if (!props.exts || !Array.isArray(props.exts)) return []

  const k = props.extsStore.exts_k || {}
  return props.exts.map(row => ({
    ...
    holdTime: getVal(row, k, 'hold_time') || getVal(row, k, 'hold'),
    ...
    caseId: getVal(row, k, 'case_id') || getVal(row, k, 'caseid'),
    disposition: getVal(row, k, 'dispositions') || getVal(row, k, 'disposition'),
    qaScore: getVal(row, k, 'qa_score') || getVal(row, k, 'qa')
  }))
})
