<template>
  <div class="flex flex-col h-full bg-gray-50 dark:bg-black p-4 gap-4">
    ...
    <div class="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-2">Internal Calls</h1>
        <p class="text-gray-500 text-sm">Monitor and manage internal/local call interactions.</p>
      </div>
    </div>

    <!-- Filters -->
    <LocalsFilter @filter="applyFilters" />
    ...
    <div class="flex-1 flex flex-col gap-4 min-h-0">
      ...
      <div class="flex flex-wrap items-center justify-between gap-4">
        <div class="flex gap-3">
          ...
          <button v-if="authStore.canExport" @click="handleDownload"
            :disabled="localsStore.loading || localsStore.locals.length === 0" :class="[
              'px-5 py-2.5 rounded-xl font-medium transition-all duration-200 flex items-center gap-2 text-sm border disabled:opacity-50 disabled:cursor-not-allowed',
              isDarkMode
      ...
      <div v-if="activeView === 'list'">
        <LocalsTable :locals="localsStore.locals" :locals-store="localsStore" />
      </div>
      ...
      <!-- Pagination Controls -->
      <Pagination v-if="activeView === 'list'" :paginationInfo="localsStore.paginationInfo"
        :hasNextPage="localsStore.hasNextPage" :hasPrevPage="localsStore.hasPrevPage" :loading="localsStore.loading"
...
import LocalsTable from "@/components/locals/LocalsTable.vue"
import LocalsFilter from "@/components/locals/LocalsFilter.vue"
...
onMounted(async () => {
  try {
    await localsStore.listLocals({ _o: 0, _c: selectedPageSize.value })
  } catch (err) {
    console.error("Error fetching internal calls:", err)
    toast.error('Failed to load internal calls data.')
  }
})
...
async function applyFilters(filters) {
  ...
  try {
    localsStore.resetPagination()
    await localsStore.listLocals({ ...filters, _o: 0, _c: selectedPageSize.value })
  ...
  try {
    if (activeView.value === 'reports') {
      ...
      const params = {
        ...
        _o: localsStore.pagination.offset,
        _c: localsStore.pagination.limit
      }
      await localsStore.listLocals(params)
    }
...
async function loadReport() {
  ...
  try {
    const data = await localsStore.getPivotReport(currentFilters.value)
...
async function handleDownload() {
  try {
    const blob = await localsStore.downloadCSV(currentFilters.value)
    const url = window.URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = 'internal_calls.csv'
    a.click()
...
async function goToNextPage() {
  try {
    await localsStore.nextPage(currentFilters.value)
  } catch (err) {
...
async function goToPrevPage() {
  try {
    await localsStore.prevPage(currentFilters.value)
  } catch (err) {
...
async function goToPage(page) {
  if (page === '...') return
  try {
    await localsStore.goToPage(page, currentFilters.value)
  } catch (err) {
...
async function changePageSize(size) {
  ...
  try {
    await localsStore.setPageSize(size, currentFilters.value)
  } catch (err) {
