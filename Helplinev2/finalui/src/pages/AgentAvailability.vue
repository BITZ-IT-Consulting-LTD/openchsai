<template>
  <div class="flex flex-col h-full bg-gray-50 dark:bg-black p-4 gap-4">
    ...
    <div class="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-2">Agent Availability</h1>
        <p class="text-gray-500 text-sm">Monitor agent occupancy and availability metrics.</p>
      </div>
    </div>

    <!-- Filters -->
    <ChanssFilter @filter="applyFilters" />
    ...
    <div class="flex-1 flex flex-col gap-4 min-h-0">
      ...
      <div class="flex flex-wrap items-center justify-between gap-4">
        <div class="flex gap-3">
          ...
          <button v-if="authStore.canExport" @click="handleDownload"
            :disabled="chanssStore.loading || chanssStore.chanss.length === 0" :class="[
              'px-5 py-2.5 rounded-xl font-medium transition-all duration-200 flex items-center gap-2 text-sm border disabled:opacity-50 disabled:cursor-not-allowed',
              isDarkMode
      ...
      <div v-if="activeView === 'list'">
        <ChanssTable :chanss="chanssStore.chanss" :chanss-store="chanssStore" />
      </div>
      ...
      <!-- Pagination Controls -->
      <Pagination v-if="activeView === 'list'" :paginationInfo="chanssStore.paginationInfo"
        :hasNextPage="chanssStore.hasNextPage" :hasPrevPage="chanssStore.hasPrevPage" :loading="chanssStore.loading"
...
import ChanssTable from "@/components/chanss/ChanssTable.vue"
import ChanssFilter from "@/components/chanss/ChanssFilter.vue"
import Pagination from "@/components/base/Pagination.vue"
import { useChanssStore } from "@/stores/chanss"
import { useAuthStore } from "@/stores/auth"

const isDarkMode = inject('isDarkMode')

const chanssStore = useChanssStore()
...
onMounted(async () => {
  try {
    await chanssStore.listChanss({ _o: 0, _c: selectedPageSize.value })
  } catch (err) {
    console.error("Error fetching agent availability:", err)
    toast.error('Failed to load agent availability data.')
  }
})
...
async function applyFilters(filters) {
  ...
  try {
    chanssStore.resetPagination()
    await chanssStore.listChanss({ ...filters, _o: 0, _c: selectedPageSize.value })

    // If on reports tab, also refresh report
  ...
  try {
    if (activeView.value === 'reports') {
      ...
      const params = {
        ...
        _o: chanssStore.pagination.offset,
        _c: chanssStore.pagination.limit
      }
      await chanssStore.listChanss(params)
    }
...
async function loadReport() {
  ...
  try {
    const data = await chanssStore.getPivotReport(currentFilters.value)
    reportData.value = data

    // Parse report data - handle common response formats
...
async function handleDownload() {
  try {
    const blob = await chanssStore.downloadCSV(currentFilters.value)
    const url = window.URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = 'agent_availability.csv'
    a.click()
...
async function goToNextPage() {
  try {
    await chanssStore.nextPage(currentFilters.value)
  } catch (err) {
...
async function goToPrevPage() {
  try {
    await chanssStore.prevPage(currentFilters.value)
  } catch (err) {
...
async function goToPage(page) {
  if (page === '...') return
  try {
    await chanssStore.goToPage(page, currentFilters.value)
  } catch (err) {
...
async function changePageSize(size) {
  ...
  try {
    await chanssStore.setPageSize(size, currentFilters.value)
  } catch (err) {
