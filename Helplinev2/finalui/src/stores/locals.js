...
export const useLocalsStore = defineStore('locals', {
  ...
  actions: {
    ...
    // Parse locals_ctx to update pagination state
    ...
    // List internal calls with pagination
    async listLocals(params = {}) {
      this.loading = true;
      this.error = null;
      ...
      try {
        ...
        const { data } = await axiosInstance.get('api/locals/', {
          params: paginationParams,
          headers: this.getAuthHeaders()
        });
        console.log('Locals API Response:', data);
        this.locals = data.locals || [];
        this.locals_k = data.locals_k || {};
        this.locals_ctx = data.locals_ctx || [];

        // Parse pagination context from response
        this.parsePaginationContext(data.locals_ctx);
      } catch (err) {
    ...
    async nextPage(filters = {}) {
      ...
      await this.listLocals({ ...filters, _o: newOffset, _c: this.pagination.limit });
    },
    ...
    async setPageSize(size, filters = {}) {
      ...
      await this.listLocals({ ...filters, _o: 0, _c: size });
    },
    ...
    async downloadCSV(params = {}) {
      try {
        const response = await axiosInstance.get('api/locals/', {
          params: { ...params, csv: 1 },
          headers: this.getAuthHeaders(),
          responseType: 'blob'
        });
    ...
    async getPivotReport(params = {}) {
      try {
        const { data } = await axiosInstance.get('api/local_rpt/', {
          params,
          headers: this.getAuthHeaders()
        });
        return data;
