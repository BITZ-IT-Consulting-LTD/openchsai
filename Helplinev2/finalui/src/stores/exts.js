...
export const useExtsStore = defineStore('exts', {
  state: () => ({
    exts: [],
    exts_k: {},
    exts_ctx: [],
  ...
  actions: {
    ...
    // Parse exts_ctx to update pagination state
    ...
    // List extension history with pagination
    async listExts(params = {}) {
      this.loading = true;
      this.error = null;
      ...
      try {
        ...
        const { data } = await axiosInstance.get('api/exts/', {
          params: paginationParams,
          headers: this.getAuthHeaders()
        });
        console.log('Exts API Response:', data);
        this.exts = data.exts || [];
        this.exts_k = data.exts_k || {};
        this.exts_ctx = data.exts_ctx || [];

        // Parse pagination context from response
        this.parsePaginationContext(data.exts_ctx);
      } catch (err) {
    ...
    async nextPage(filters = {}) {
      ...
      await this.listExts({ ...filters, _o: newOffset, _c: this.pagination.limit });
    },
    ...
    async setPageSize(size, filters = {}) {
      ...
      await this.listExts({ ...filters, _o: 0, _c: size });
    },
    ...
    async downloadCSV(params = {}) {
      try {
        const response = await axiosInstance.get('api/exts/', {
          params: { ...params, csv: 1 },
          headers: this.getAuthHeaders(),
          responseType: 'blob'
        });
    ...
    async getPivotReport(params = {}) {
      try {
        const { data } = await axiosInstance.get('api/ext_rpt/', {
          params,
          headers: this.getAuthHeaders()
        });
        return data;
